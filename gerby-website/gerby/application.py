...
def post_payload():
  ...
  signature = hmac.new(bytes(SECRECT_TOKEN, 'latin-1'), msg=bytes(req_body, 'latin-1'), digestmod=hashlib.sha256).hexdigest()
  print(request.headers['X-Hub-Signature-256'])
  print("Do signatures match?", hmac.compare_digest(bytes(signature, 'latin-1'), request.headers['X-Hub-Signature-256']))
