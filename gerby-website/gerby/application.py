...
def get_statistics():
    ...
    if BookStatistic.table_exists():
        try:
            statistics.append(str(BookStatistic.get(
                BookStatistic.statistic == "pages").value) + " pages")
        except BookStatistic.DoesNotExist:
            app.logger.warning("No entry 'pages' in table 'BookStatistics'.")

        try:
            statistics.append(str(BookStatistic.get(
                BookStatistic.statistic == "lines").value) + " lines of code")
        except BookStatistic.DoesNotExist:
    ...
    statistics.append(str(Tag.select().where(
        Tag.type == "section").count()) + " sections")
    statistics.append(str(Tag.select().where(
        Tag.type == "chapter").count()) + " chapters")
    ...
    for label, feed in feeds.items():
        ...
        if not os.path.isfile(path) or time.time() - os.path.getmtime(path) > 3600:
            try:
                ...
                app.logger.warning(
                    "feed '%s' did not load properly" % feed["title"])
...
def show_index():
    ...
    if Comment.table_exists():
        comments = Comment.select().where(Comment.active).order_by(
            Comment.id.desc()).paginate(1, 5)
...
def show_statistics():
    ...
    records = dict()
    records["complex"] = TagStatistic.select(TagStatistic.tag, fn.MAX(TagStatistic.value).alias(
        "value")).where(TagStatistic.statistic == "preliminaries").execute()[0]
    records["used"] = TagStatistic.select(TagStatistic.tag, fn.MAX(TagStatistic.value).alias(
        "value")).where(TagStatistic.statistic == "consequences").execute()[0]
    records["referenced"] = Dependency.select(Dependency.to, fn.COUNT(Dependency.to).alias(
        "value")).group_by(Dependency.to).order_by(fn.COUNT(Dependency.to).desc())[0]
    records["proof"] = Proof.select(Proof.tag, fn.length(Proof.html).alias(
        "value")).order_by(fn.length(Proof.html).desc())[0]
...
def show_chapters():
    ...
    if Tag.select().where(Tag.type == "part").exists():
        ...
        for part in parts:
            part.chapters = sorted(
                [chapter.chapter for chapter in chapters if chapter.part.tag == part.tag])
...
def post_payload():
    ...
    signature = 'sha256=' + \
        hmac.HMAC(key=SECRET_TOKEN.encode(), msg=req_body,
                  digestmod=hashlib.sha256).hexdigest()
    verified = hmac.compare_digest(bytes(
        signature, 'utf-8'), bytes(request.headers['X-Hub-Signature-256'], 'utf-8'))
