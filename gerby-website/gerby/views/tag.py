...
def getNeighbours(tag):
    ...
    try:
        if tag.type in headings:
            # to deal with parts appropriately we select for part / chapter with the appropriate number
            left = Tag.get(Tag.ref == left, Tag.type == tag.type)
        else:
...
def show_tag(tag):
    ...
    if tag.type in headings:
        ...
        if headings.index(tag.type) >= headings.index(gerby.configuration.UNIT):
            ...
            for reference in references:
                html = html.replace("id=\"" + reference.tag.tag + "\">", 'id="' + reference.tag.tag +
                                    '"><a tabindex="0" role="button" data-trigger="focus" data-placement="bottom" class="btn badge badge-info" data-toggle="popover" title="Reference" data-html="true" data-content=\'' + reference.html.replace("'", "&#39;") + '\'>reference</a>')
            histories = History.select().where(History.tag << identifiers)
            for history in histories:
                html = html.replace("id=\"" + history.tag.tag + "\">", 'id="' + history.tag.tag +
                                    '"><a tabindex="0" role="button" data-trigger="focus" data-placement="bottom" class="btn badge badge-secondary" data-toggle="popover" title="Historical remark" data-html="true" data-content=\'' + history.html.replace("'", "&#39;") + '\'>historical remark</a>')
            slogans = Slogan.select().where(Slogan.tag << identifiers)
            for slogan in slogans:
                html = html.replace("id=\"" + slogan.tag.tag + "\">", 'id="' + slogan.tag.tag +
                                    '"><a tabindex="0" role="button" data-trigger="focus" data-placement="bottom" class="btn badge badge-primary" data-toggle="popover" title="Slogan" data-html="true" data-content=\'' + slogan.html.replace("'", "&#39;") + '\'>slogan</a>')
    ...
    # it's an item
        ...
        # <ol start> is incompatible with our current counter setup
        html = "<ul>" + tag.html + "</ul>"

    # it's a tag (maybe with proofs)
    else:
        html = tag.html + \
            "".join([proof.html for proof in tag.proofs.order_by(Proof.number)])
    ...
    for number, label in enumerate(labels):
        raw_mark_search = re.search(r"id=\"" + label + "-mark\"><sup>([0-9]+)</sup>", html)
        if not raw_mark_search:
            continue

        old = raw_mark_search.group(1)
        html = html.replace(
        ...
        footnote_num_str = "<a href=\"#" + label + \
            "\"><sup>" + str(number + 1) + "</sup></a>"
        insert_idx = html.find(footnote_num_str) + len(footnote_num_str)
        html = html[:insert_idx] + "<span class=\"sidenote\">" + \
            footnotes[number].html + "</span>" + html[insert_idx:]
    ...
    html = html.replace("class=\"footnotemark\"",
                        "class=\"footnotemark footnote-offset\"")
    ...
    if tag.type in headings and headings.index(tag.type) < headings.index(gerby.configuration.UNIT):
        ...
        if tag.type == "part":
            ...
            chapters = Tag.select().where(
                Tag.tag << [chapter.chapter.tag for chapter in chapters])

            prefixes = tuple(chapter.ref + "." for chapter in chapters)
            sections = Tag.select().where(Tag.type == "section")
            sections = filter(
                lambda section: section.ref.startswith(prefixes), sections)
    ...
    if commentsEnabled:
        ...
        for parent in breadcrumb:
            if parent.tag == tag.tag:
                continue
            count = Comment.select().where(Comment.tag == parent.tag,
                                           Comment.active).count()  # this could be done in a single JOIN
            if count > 0:
...
def send_citation_info(tag):
    if not isTag(tag):
        return {'doesExist': False, 'tag': None}

    try:
        tag = Tag.get(Tag.tag == tag.upper())
    except Tag.DoesNotExist:
        return {'doesExist': False, 'tag': None}
    return {'doesExist': True, 'tag': tag.tag}
...
def show_tag_statistics(tag):
    ...
    try:
        ...
        creation = datetime.datetime.strptime(
            creation.commit.time.decode(), "%Y-%m-%d %H:%M:%S %z")

        update = Commit.select().where(Commit.hash << [
            change.commit.hash for change in tag.changes]).order_by(Commit.time.desc()).get()
        update = datetime.datetime.strptime(
            update.time.decode(), "%Y-%m-%d %H:%M:%S %z")
    ...
    if TagStatistic.table_exists():
        for statistic in ["preliminaries", "chapters", "sections", "consequences"]:
            try:
                statistics[statistic] = tag.statistics.filter(
                    TagStatistic.statistic == statistic).get().value
            except TagStatistic.DoesNotExist:
                log.warning(
                    "Unable to get statistic '{0}' for tag '{1}'.".format(statistic, tag))
