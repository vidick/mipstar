...
The classical value $\val(\game_\CHSH)=\frac{3}{4}$. The tensor product and commuting values $\val^*(\game_\CHSH)=\val^c(\game_\CHSH)=\cos^2 \frac{\pi}{8}$. The non-signaling value $\val^{ns}(\CHSH)=1$. 
\end{lemma}
...
\end{align}
Here the second line follows by expanding all squares, using that each operator squares to identity, and $A$ and $B$ operators commute. The last term above has norm at most $8$, whcih shows that $A_0 B_0 + A_0 B_1 + A_1 B_0 - A_1 B_1 $ has norm at most $\sqrt{8}$; the claimed bound on the commuting value of the CHSH game  follows. Reference~\ref{equation-bellop}.
