...
The following diagram intuitively captures the situation described in Lemma~\ref{lemma-ac-exact}. 
...
The proof of Lemma~\ref{lemma-ac-exact} makes use of Jordan's lemma (also known as the ``CS decomposition''), which we restate as follows. 
...
\end{equation}
for all $a,b,c\in\{0,1\}$. It is immediate to verify that for all $(x,y)\in \mP$ we have $f(x)^*f(y)=f(x^{-1}y)$ and so~\eref{equation-gh-condition} holds with $\eps=0$ for any $\sigma$.
\end{example}
...
\begin{proof}[Proof of Theorem \ref{theorem-gh}]
...
\end{eqnarray*}
where for the second line we used the definition \eref{equation-fourier} of $\hat{f}(\rho)$ and  for the third we used \eref{equation-ortho} and the fact that $f$ takes values in the unitary group.
...
&& =  \Es{z}\,  f(z)^*f(zx) , 
\end{eqnarray*}
where the last equality uses \eref{equation-ortho}.
